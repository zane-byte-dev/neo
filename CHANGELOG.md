...
- **文章内资源 MVP**：文章编辑器新增“相关资源”状态带和正文底部“本篇资源”预览区，可在文章内发现、生成、预览并直接打开音频概览、思维导图和报告；新 artifact 会持久化 `sourceIds` / `primaryArticleId`，`ResourcesPanel` 继续作为 notebook 级资源库与管理入口。
