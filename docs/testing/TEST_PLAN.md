...
前置: 每个测试用例为测试用户配置独立 `workDir`，并在测试环境显式设置 `process.env.USERS`

存储路径：`{stateDir}/projects/chat-sessions.json` 与 `{stateDir}/projects/{sessionId}/chat-{sessionId}.jsonl`
...
| 模块 | 问题 | 建议 |
|------|------|------|
| `chat-service.ts` | 已迁移到用户 `workDir`，但测试仍依赖 `process.env.USERS` 和测试用户配置 | 提供统一的 test helper，为测试用户注入 `workDir` 和 `USERS` |
