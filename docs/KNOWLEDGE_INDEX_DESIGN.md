...
1. 定义 `KnowledgeDocument` / `KnowledgeChunk` / `KnowledgeHit` 类型
2. 抽象 `KnowledgeStore` 接口
3. 让 Notebook chat 和 memory recall 先通过接口调用，即使底层仍旧各走各的
...
1. 引入 SQLite 存储层
2. 为 notebook source / notebook note / semantic memory 建立 document + chunk + FTS 表
3. 增加索引任务队列与 backfill 命令
...
1. Notebook chat 改为从统一索引层取 chunk
2. citation 附带 `chunkId + charStart + charEnd`
3. SourceDetailView 支持按偏移滚动定位
...
1. Notebook source 与 semantic memory 已能被同一接口检索
2. Notebook chat 可以返回带偏移的引用命中
3. 索引库删除后可由事实源完全重建
