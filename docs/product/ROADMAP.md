...
- [x] **文章批注 MVP**：文章编辑器支持选区批注，批注以独立 annotation 数据模型持久化；正文用下划线标记，hover 时在文档右侧显示批注卡片，可查看、跳转、删除，并切换未解决 / 已解决状态
...
- [x] **批注辅助面板**：按文章顺序列出所有批注，支持 open/resolved 筛选和点击跳转正文位置；当前以文档右侧 rail 形式提供（P1）
...
2. **最高优先级缺口（2026-05-16）**：
   - **批注辅助面板**（F14）：已补齐最小可用版（全部批注、状态/类型筛选、点击跳转），后续继续推进段落批注入口
...
6. **技术债务优先级**：delete route 回归测试（B9）与文档断链（B10）已恢复，后续继续守住 CI 全绿状态
