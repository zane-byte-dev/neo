...
**推进状态：** 2026-05-12 已完成一轮文档补齐与入口同步
...
3. 发送第一条消息 → 成功 ✅
4. 想让 AI 调用工具帮我处理文件 → README + `docs/TOOLS.md` 能找到工具和自定义工具协议 ✅
5. 想建立知识库 → `docs/NOTEBOOK.md` 说明来源导入、Studio、引用模式 ✅
6. 想写一个自定义 Skill → `docs/SKILLS.md` + `examples/skills` 可直接参考 ✅
7. 想用 Docker 沙箱执行代码 → `docs/SANDBOX.md` 说明模式、环境变量和产物输出 ✅
8. 想接入 MCP 服务器 → `docs/MCP.md` 说明 `mcp.json` 和工具前缀 ✅
9. 遇到问题想查日志 → `docs/FAQ.md` 和 CONTRIBUTING 调试章节有入口 ✅
10. 想了解 Telegram Bot 如何工作 → README + FAQ 覆盖绑定、Token、userId 排查；命令列表仍可后续深化 🔄
```

**推进后用户旅程成功率：9/10（剩余主要是 Telegram 进阶命令和真实截图资产）**
...
| **完整度** | 8.5 | 关键功能文档已补齐；Telegram 进阶和截图仍可增强 |
| **双语一致性** | 7.5 | 英文 README 已同步启动、布局和指南入口；详细指南仍以中文为主 |
| **维护性** | 8.0 | PR checklist + CHANGELOG 已建立；工具文档自动生成仍待做 |
| **视觉呈现** | 7.0 | README 已补真实 Web UI 截图；后续可继续补演示 GIF |
| **综合** | **8.4** | 核心文档剪刀差已收敛，剩余问题集中在自动化同步机制和大规模命名规范 |
...
### 近期迭代（2 周内）
- [x] 新增 `docs/TOOLS.md` 内置工具参考手册
- [x] 新增 `docs/SKILLS.md` 技能开发指南
- [x] 新增 `docs/SANDBOX.md` 沙箱使用指南
- [x] 新增 `docs/MCP.md` MCP 配置指南
- [x] README 英文版同步首次启动、布局和指南入口

### 规划迭代（1 个月内）
- [x] 新增 `docs/NOTEBOOK.md` 知识库使用指南
- [x] 新增 `docs/AUTOMATION.md` 自动化指南
- [x] 重组 `docs/` 索引为「用户指南 / 开发者指南 / 产品与规划 / 测试与质量」分层
- [x] 在 PR 模板中加入文档 checklist
- [x] README 增加真实截图 / 演示 GIF
- [x] 建立 CHANGELOG.md

---

> **推进后总结**：Neo 的文档已经从“能启动”推进到“能深度用”的基础状态。核心能力均已有入口文档，README 与英文版的关键差异已收敛，PR checklist 与 CHANGELOG 也补上了维护机制。下一步最值得做的是补真实产品截图 / 演示 GIF，并把工具参考从源码自动生成，进一步降低文档漂移风险。
