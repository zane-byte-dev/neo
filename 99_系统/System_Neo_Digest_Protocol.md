...
> **核心目标**：实现 Inbox (00_收集) 零存留（需排除 `00_收集/Archive/` 目录），所有入库信息必须在 24 小时内被分配到具体的知识维度。
...
| `#archive`, `#归档` | `00_收集/Archive/` | 直接移动，不进行 AI 处理 |
...
4.  **忽略归档**：在扫描和处理 `00_收集/` 目录时，必须**严格跳过** `00_收集/Archive/` 及其子目录中的所有内容。
