...
export async function streamNotebookChat(
    ...
    nbAppendChatMessage(workDir, notebook, userEntry, stateDir);
    ...
    const history = nbReadChatHistory(workDir, notebook, stateDir);
    ...
    try {
        ...
        if (out?.usage && userId) {
            void appendUsageRecord({
                ...
                systemPrompt,
            }, stateDir).catch(() => { /* never crash over tracking */ });
        }
    } catch (err) {
    ...
    nbAppendChatMessage(workDir, notebook, assistantEntry, stateDir);
