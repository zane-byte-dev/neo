...
async function refreshUser(userId: string, stateDir: string): Promise<void> {
    const dailyDir = join(stateDir, 'memory', '1-Daily');
    const nowPath  = join(stateDir, 'memory', 'NOW.md');
...
export async function refreshNowForAllUsers(): Promise<void> {
    ...
    const users = userList().filter((u) => u.stateDir );

    for (const user of users) {
        try {
            await refreshUser(user.id, user.stateDir!);
        } catch (err) {
