...
// Sub-resources are stored under `{stateDir}/notebooks/{name}/` so they don't mix
// with the user's own article .md files:
//
//   notebooks/{name}/meta/config.json               — notebook settings
//   notebooks/{name}/meta/source-guides/{id}.json   — AI-generated summary
//   notebooks/{name}/notes/{noteId}.md              — user/AI notes
//   notebooks/{name}/artifacts/{artifactId}.json    — generated artifacts
//   notebooks/{name}/chat/history.jsonl             — notebook-scoped chat
//
...
/** System metadata directory for a notebook (inside stateDir). */
function notebookBaseDir(stateDir: string, notebook: string): string {
    return join(stateDir, 'notebooks', notebook);
}

function notebookMetaDir(stateDir: string, notebook: string): string {
    return join(notebookBaseDir(stateDir, notebook), 'meta');
}

function sourceGuideDir(stateDir: string, notebook: string): string {
    return join(notebookMetaDir(stateDir, notebook), 'source-guides');
}

function sourceGuideFilePath(stateDir: string, notebook: string, sourceId: string): string {
    return join(sourceGuideDir(stateDir, notebook), `${safeFilename(sourceId)}.json`);
}

function notebookConfigFilePath(stateDir: string, notebook: string): string {
    return join(notebookMetaDir(stateDir, notebook), 'config.json');
}

function notebookNotesDir(stateDir: string, notebook: string): string {
    return join(notebookBaseDir(stateDir, notebook), 'notes');
}

function notebookNoteFilePath(stateDir: string, notebook: string, noteId: string): string {
...
function notebookArtifactFilePath(stateDir: string, notebook: string, artifactId: string): string {
    return join(notebookArtifactsDir(stateDir, notebook), `${safeFilename(artifactId)}.json`);
}

function notebookChatDir(stateDir: string, notebook: string): string {
    return join(notebookBaseDir(stateDir, notebook), 'chat');
}
...
export function nbGetSourceGuide(workDir: string, notebook: string, sourceId: string, stateDir = workDir): SourceGuide | undefined {
    const file = sourceGuideFilePath(stateDir, notebook, sourceId);
...
export function nbListSourcesWithGuides(workDir: string, notebook: string, stateDir = workDir): (SourceMeta & { guide: SourceGuide | null })[] {
    const sources = nbListSources(workDir, notebook);
    return sources.map((s) => {
        const file = sourceGuideFilePath(stateDir, notebook, s.id);
...
export function nbSaveSourceGuide(workDir: string, notebook: string, guide: SourceGuide, stateDir = workDir): void {
...
export function nbGetConfig(workDir: string, notebook: string, stateDir = workDir): NotebookConfig {
    const file = notebookConfigFilePath(stateDir, notebook);
...
export function nbListNotes(workDir: string, notebook: string, stateDir = workDir): NotebookNote[] {
    const dir = notebookNotesDir(stateDir, notebook);
    if (!existsSync(dir)) return [];
...
export function nbSaveNote(workDir: string, notebook: string, data: NoteSaveInput, stateDir = workDir): NotebookNote {
    const dir = notebookNotesDir(stateDir, notebook);
    ensureDir(dir);
    const now = Date.now();
...
export function nbDeleteNote(workDir: string, notebook: string, noteId: string, stateDir = workDir): boolean {
    const dir = notebookNotesDir(stateDir, notebook);
    const file = notebookNoteFilePath(stateDir, notebook, noteId);
...
export function nbConvertNoteToSource(workDir: string, notebook: string, noteId: string, stateDir = workDir): SourceMeta | undefined {
    ...
    nbDeleteNote(workDir, notebook, noteId, stateDir);
    return imported;
}
...
export function nbListArtifacts(workDir: string, notebook: string, type?: ArtifactType, stateDir = workDir): Artifact[] {
    const dir = notebookArtifactsDir(stateDir, notebook);
    if (!existsSync(dir)) return [];
...
export function nbGetArtifact(workDir: string, notebook: string, id: string, stateDir = workDir): Artifact | undefined {
    const file = notebookArtifactFilePath(stateDir, notebook, id);
    if (!existsSync(file)) return undefined;
...
export function nbDeleteArtifact(workDir: string, notebook: string, id: string, stateDir = workDir): boolean {
    const file = notebookArtifactFilePath(stateDir, notebook, id);
    if (!existsSync(file)) return false;
    unlinkSync(file);
    return true;
...
function chatFilePath(stateDir: string, notebook: string): string {
    return join(notebookChatDir(stateDir, notebook), 'history.jsonl');
}

export function nbReadChatHistory(workDir: string, notebook: string, stateDir = workDir): NotebookChatMessage[] {
    const file = chatFilePath(stateDir, notebook);
    if (!existsSync(file)) return [];
    return parseJsonLines<NotebookChatMessage>(readFileSync(file, 'utf8'));
}

export function nbAppendChatMessage(workDir: string, notebook: string, msg: NotebookChatMessage, stateDir = workDir): void {
 ...
 */
export function nbForkChatHistory(workDir: string, notebook: string, messageId: string, stateDir = workDir): NotebookChatMessage[] {
    const all = nbReadChatHistory(workDir, notebook, stateDir);
