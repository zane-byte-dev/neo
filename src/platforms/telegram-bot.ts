...
import { getTelegramBotToken, getTelegramChatId } from '../config.js';
...
function resolveTelegramUserId(chatId: string): string | null {
    ...
    if (telegramChatId && telegramChatId === chatId) {
    ...
    async function handleAgentTurn(
        ...
        void (async () => {
            ...
            try {
                ...
                let userCtxPromise: Promise<Awaited<ReturnType<typeof calcUser>>> | null = null;
                const getUserCtx = () => {
                    userCtxPromise ??= calcUser(userId);
                    return userCtxPromise;
                };
                const output = await runAgentTurn({
                    ...
                    onImage: async (data: string, _mimeType: string, caption?: string) => {
                        ...
                        return persistImageArtifact(userCtx.stateDir ?? userCtx.workDir, runId, data, _mimeType, caption);
                    },
                ...
                log.info(MODULE, 'Turn completed', {
                    chatId,
                    sessionId,
                    elapsed,
                ...
                const text = outcome?.responseText || output || '模型没有返回可显示内容。';
                ...
                const artifactLines = renderArtifactReferences(
                    (outcome?.artifacts ?? []).filter((artifact) => artifact.kind !== 'image'),
                );
                for (const line of artifactLines) {
                    await ctx.reply(line).catch(() => {});
                }
            } catch (err: unknown) {
                const elapsed = Date.now() - t0;
