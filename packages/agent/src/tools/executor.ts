...
export async function executeTool(
    ...
    if (context?.confirmCallback) {
        if (permission === 'dangerous') {
            const autoApprovedScope = await matchToolApprovalScope(context.stateDir ?? workDir, {
                sessionId: context.sessionId,
                toolName: name,
                args,
            });
            if (autoApprovedScope) {
                log.info('AgentRuntime', `Tool ${name} auto-approved by ${autoApprovedScope} rule`);
            } else {
            try {
