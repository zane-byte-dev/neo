/**
 * src/tools/user-tools/loader.ts — Load user-defined tools from {stateDir}/tools/.
 *
 * Scans {stateDir}/tools/{name}/tool.yaml, parses each into a Tool object
...
/**
 * Load all user tools from {stateDir}/tools/ directory.
 * Returns a Map<toolName, Tool> ready to be merged into the AI tool set.
 */
export async function loadUserTools(stateDir: string): Promise<Map<string, Tool>> {
