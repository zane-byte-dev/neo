 ...
 * Two scopes:
 *   - session (default): stored in {stateDir}/projects/{sessionId}/todos.json
...
function sessionTodosPath(stateDir: string, sessionId: string): string {
    return join(stateDir, 'projects', sessionId, 'todos.json');
}

function persistentTodosPath(stateDir: string): string {
    return join(stateDir, 'memory', 'tasks.json');
}

function todosPath(stateDir: string, sessionId: string, scope: string): string {
    return scope === 'persistent'
        ? persistentTodosPath(stateDir)
        : sessionTodosPath(stateDir, sessionId);
}

async function loadTodos(stateDir: string, sessionId: string, scope = 'session'): Promise<TodoItem[]> {
    try {
        const raw = await fs.readFile(todosPath(stateDir, sessionId, scope), 'utf-8');
...
export const todoTool: Tool = {
    ...
    handler: async (args, workDir, context) => {
        ...
        // ── LIST
        if (action === 'list') {
            const todos = await loadTodos(stateDir, sessionId, scope);
        ...
        if (action === 'write') {
            ...
            await saveTodos(stateDir, sessionId, items as TodoItem[], scope, context);
        ...
        if (action === 'update') {
            ...
            await saveTodos(stateDir, sessionId, todos, scope, context);
