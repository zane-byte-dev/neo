#!/usr/bin/env node
/**
 * butler.ts — Daily maintenance script
 *
 * Tasks:
 ...
 *
 * Called by the 02:00 AM cron job in telegram-bot.ts via:
 *   npx tsx apps/refinery/butler.ts
 */
...
/**
 * Move flat diary files (YYYY-MM-DD.md) at the root of history/ into YYYY/MM/ sub-folders.
 * Today's file and the template are excluded.
 */
async function archiveDiary(vaultRoot: string): Promise<number> {
    ...
    for (const entry of entries) {
        ...
        // Expect: YYYY-MM-DD.md (length 13)
...
/**
 * Delete empty / near-empty files anywhere under inbox/.
 */
